...
Build a simple express server. Connect a '/hello' route that sends a greeting of your  choice.
 Connect a '/aboutme' route that sends a short bio about you to the front-end. Finally, connect
  a '/favoritefoods' route that sends an array to the front-end of your favorite foods. All
   other routes should respond with a status of 404.
------------------------------------------------------------------------------------------------ */
...
const createServer = () => {
    ...
    function sayHello(request, response) {
        response.status(200).send("Hi! what's going on?");
    };
    app.get('/aboutme', (request, response) => {
        response.status(200).send("I want to use code to simplify peoples lives with the idea of giving more people the time to be present with themselves with each other");
    });
    app.get('/favoritefoods', (request, response) => {
        response.status(200).send(['thai toms', "loretta's northwesterne", 'sisters and brothers', 'big marios']);
    });
...
Write a function named count that, given an integer and an array of arrays, uses either filter, map, or 
reduce to count the amount of times the integer is present in the array of arrays.
...
const count = (target, input) => {
    const johnson = [];
    const jelly = input.map( array => {
      let jim = array.reduce((accumulator, value, index) =>{
        if(target === value){
          johnson.push(value);
        }
    },0);
...
const totalSum = (input) => {
    let folly = input.reduce((acc, value, index) => {
      return acc + value.reduce((accumulator, value, index) => {
...
let divisibleByFiveTwoToThePower = (arr) => {
    let funPuzzles = arr.map(singleArr => {
  let divByFive = singleArr.reduce((answerSoFar, value) => {
