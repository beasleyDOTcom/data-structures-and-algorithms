...
const templateWithJQuery = () => {
    ...
    starWarsPeople.forEach((person)  => {
        // get html template
        //create section to fill
        const template = $("#template").html();
        const $section = $('<section></section>');
        $section.html(template);
       $section.find("h2").append(person.name);
       $section.find("h3").append(person.height);
       $section.find("p").append(person.eye_color);
       $("main").append($section);
    });
}
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach((list) => {
      let index = list.indexOf(' ', list.indexOf(' ') +1) + 1;
      result.push(list.slice(index, list.length));
  });
  return result;
};
